<script lang="ts">
	...
	$: _time = new Date(
		time.getTime() + time.getTimezoneOffset() * 60 * 1000 + offset * 60 * 60 * 1000
	); // Convert Timezone
	...
	$: {
		if (timezoneSearch) {
			let tmp: [number, Timezone][] = _timezones.map((t) => [score(t, timezoneSearch), t]);
	...
	function score(timezone: Timezone, query: string): number {
		...
		return (
			((_query.startsWith("utc") && _query.slice(3) === clean(timezone.offset.toString())) ||
			_query === clean(timezone.offset.toString())
				? 10
				: 0) +
	...
	function onWindowClick(e: WithTarget<MouseEvent, Window>) {
		...
		while (target && target !== root) target = target.parentNode as HTMLElement;
